...
/* globals document, lekoLineDict, alert, console */
...
function makeElement(elementTemplateObject) {
  ...
  if (elementType == undefined){
    alert("error! an invalid object was passed to makeElement():\n" + JSON.stringify(elementTemplateObject));
    return;
  }
...
function makePaths(multiArray, styleDict) {
  ...
  let properties = smooshObjects(pathDefaults, styleDict);

  let tempString = "";
  for (let pathArr of multiArray){
...
function makeSimplePath(multiArray, tail) {
  let result = JSON.stringify(multiArray);
...
function forceLekoCompat(input) {
  ...
  if (typeof input == "string"){
    ...
    alert("error, unknown input type in forceLekoCompat(): " + JSON.stringify(input));
  }
  //unused, todo update and add test
  let resultArr = [];
...
function assembleSVG(input, calligraphyStyleObj) {
  ...
  for (let group of resultObj._groups){
